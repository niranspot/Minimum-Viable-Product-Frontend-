...
const ROLE_DETAILS = {
  doctor:     { color: '#0d9488', bgColor: '#e6f7f5', icon: <LocalHospital fontSize="small" />, label: 'Doctor' },
  nurse:      { color: '#6366f1', bgColor: '#eef2ff', icon: <Person fontSize="small" />,        label: 'Nurse' },
  admin:      { color: '#ef4444', bgColor: '#fef2f2', icon: <AdminPanelSettings fontSize="small" />, label: 'Admin' },
  pharmacist: { color: '#f59e0b', bgColor: '#fffbeb', icon: <MedicationLiquid fontSize="small" />, label: 'Pharmacist' },
};

const getRoleDetails = (role) =>
  ROLE_DETAILS[role] || { color: '#6b7280', bgColor: '#f3f4f6', icon: <Person fontSize="small" />, label: role || 'Staff' };
    ...
    <>
      <Box sx={{ p: { xs: 2, sm: 3 } }}>
        <Paper elevation={0} sx={{ borderRadius: 3, overflow: 'hidden', border: '1px solid', borderColor: 'divider', mb: 3 }}>
          <Box sx={{ p: 3, background: 'linear-gradient(135deg, #f8fafc 0%, #eef2ff 100%)', borderBottom: '1px solid', borderColor: 'divider' }}>
            <Stack direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between" alignItems="flex-start" spacing={2}>
              <Box>
                <Stack direction="row" spacing={1} alignItems="center">
                  <LocalHospital sx={{ color: '#0d9488', fontSize: 28 }} />
                  <Typography variant="h5" fontWeight={700}>Staff Directory</Typography>
                  <Chip label={`${stats.total} total`} size="small" sx={{ bgcolor: '#0d9488', color: 'white', fontWeight: 600, fontSize: '0.7rem', ml: 1 }} />
                </Stack>
          ...
          <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr 1fr', sm: 'repeat(4, 1fr)' }, gap: 2, p: 3 }}>
            <StatCard icon={<Person />} label="Total Staff" value={stats.total} color="#0d9488" />
            <StatCard icon={<CheckCircle />} label="Active" value={stats.active} color="#22c55e" />
            <StatCard icon={<Cancel />} label="Pending/Suspended" value={stats.inactive} color="#ef4444" />
            <StatCard icon={<LocalHospital />} label="Doctors" value={stats.doctors} color="#6366f1" />
          </Box>
        </Paper>

        <Card elevation={0} sx={{ border: '1px solid', borderColor: 'divider', borderRadius: 3, overflow: 'hidden' }}>
          <Box sx={{ p: 2, borderBottom: '1px solid', borderColor: 'divider', bgcolor: '#fafbfc' }}>
                ...
                InputProps={{
                  ...
                  endAdornment: searchTerm && (
                    <InputAdornment position="end">
                      <IconButton size="small" onClick={() => setSearchTerm('')}><Cancel fontSize="small" /></IconButton>
                    </InputAdornment>
                  )
                }}
              />
                  ...
                  <TableRow>
                    <TableCell colSpan={5} align="center" sx={{ py: 8 }}>
                      <Stack spacing={2} alignItems="center">
                        <Person sx={{ fontSize: 48, color: '#cbd5e1' }} />
                  ...
                  filteredUsers.map((user) => {
                    ...
                    return (
                      <TableRow key={user.id} sx={{ '&:hover': { bgcolor: '#f8fafc' } }}>
                        <TableCell>
                          <Stack direction="row" spacing={2} alignItems="center">
                            <MuiBadge
                              ...
                              badgeContent={
                                <Box sx={{ width: 10, height: 10, borderRadius: '50%', bgcolor: user.status === 'active' ? '#22c55e' : '#ef4444', border: '2px solid white' }} />
                              }
                            >
                              <Avatar sx={{ bgcolor: roleDetails.color, width: 40, height: 40, fontWeight: 600 }}>
                        ...
                        <TableCell>
                          <Chip icon={roleDetails.icon} label={roleDetails.label} size="small" sx={{ bgcolor: roleDetails.bgColor, color: roleDetails.color, fontWeight: 600 }} />
                        </TableCell>
                        <TableCell>
                          <Chip
          ...
          <Box sx={{ p: 2, borderTop: '1px solid', borderColor: 'divider', bgcolor: '#fafbfc' }}>
...
const StatCard = ({ icon, label, value, color }) => (
  <Box sx={{ p: 1.5, borderRadius: 2, bgcolor: `${color}10`, border: '1px solid', borderColor: `${color}20`, display: 'flex', alignItems: 'center', gap: 1.5 }}>
    <Box sx={{ p: 1, borderRadius: 1.5, bgcolor: `${color}20`, color, display: 'flex', alignItems: 'center' }}>{icon}</Box>
    <Box>
